        ...
        <div class="card">
          ...
          <div class="card-body">
            ...
            <p class="information">
              ...
              <span class="value">{{ stakeWeight | borsh(8) }}</span>
            </p>
            <p class="information">
              <span class="key">{{ $t('blockchain.fee_rate') }}</span>:
              <span class="value">{{ feeRate }} BORSH/kB</span>
            </p>
          </div>
        </div>
      </div>
      ...
      <div class="column is-half">
        <div class="card">
          <div class="card-header">
            ...
            <nuxt-link to="/block" class="card-header-button button is-borsh is-outlined">
              {{ $t('action.view_all') }}
            </nuxt-link>
          </div>
          <div class="card-body">
            <div v-for="block in recentBlocks" class="borsh-block is-size-7" :key="block.hash">
              <div class="level">
                <div class="level-left">
                  <nuxt-link :to="{name: 'block-id', params: {id: block.height}}"
                    class="level-item borsh-block-box has-text-centered">
                  ...
                  <div class="level-item">
                    <div>
                      ...
                      <span class="monospace">
                        {{ $t('block.brief.reward') }} {{ block.reward | borsh }} BORSH
                      </span>
                    </div>
                  </div>
                </div>
          ...
          <div class="card-body">
            <div v-for="transaction in recentTransactions" :key="transaction.id" class="is-size-7 transaction">
              <div class="level">
                ...
                <span class="level-right">{{ transaction.outputValue | borsh }} BORSH</span>
              </div>
            </div>
          </div>
        </div>
...
<script>
  ...
  import {RequestError} from '@/services/borshinfo-api'
