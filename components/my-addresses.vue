<template>
  <div v-show="addresses.length">
    <button class="button is-borsh" @click="show = !show">
    ...
    <table v-show="show">
      <tbody>
        <tr v-for="{address, balance} in list">
          ...
          <td class="monospace has-text-right">{{ balance | borsh(8) }}</td>
          <td>
      ...
      <tfoot v-if="addresses.length > 1">
        <tr>
          ...
          <td class="monospace has-text-right">{{ totalBalance | borsh(8) }}</td>
          <td></td>
        </tr>
      </tfoot>
    </table>
