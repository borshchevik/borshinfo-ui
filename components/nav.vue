  ...
  <nav class="navbar">
    <div class="navbar-brand is-size-4">
      <nuxt-link to="/" class="navbar-item navbar-logo">
        <span class="borsh-icon borsh-icon--borsh" /> borsh.info
      </nuxt-link>
    ...
    <div class="navbar-menu" :class="{'is-active': showMenu}">
      ...
      <form class="navbar-end" @submit.prevent="search">
        <div class="navbar-item input-item">
          ...
          <button type="submit" class="button is-borsh" :class="{'is-loading': searching}">
            <Icon icon="search" />
          </button>
        </div>
      </form>
...
<script>
  import {get as borshinfoGet} from '@/services/borshinfo-api'
  ...
  export default {
    ...
    methods: {
      async search() {
        ...
        this.searching = true
        try {
          let {type, id, address} = await borshinfoGet(`/search`, {params: {query: searchString}})
          switch (type) {
          case 'address':
            this.searchString = ''
